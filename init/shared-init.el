...
;; Workaround for bug#79687: Emacs 30.2 serialises `:match' / `:equal' /
;; `:pred' to `#match' / `#equal' / `#pred' (no trailing `?'), which
;; libtree-sitter >=0.26 rejects at query parse time. The upstream fix
;; (commit b0143530 on master, Emacs 31.x) changes both the serialiser
;; and the C dispatcher; it has not been backported to emacs-30 as of
;; 30.2. On Arch we ship libtree-sitter.so.0.26, so every `:match'-using
;; query in `python--treesit-settings' fails with
;; `treesit-query-error "Syntax error at"' the first time it hits
;; tree-sitter -- including during markdown native fontification of
;; ```python``` blocks.
;;
...
;; by `@python--treesit-fontify-string' upstream). Captures named after
;; a function are left as plain captures by `treesit-font-lock-rules',
;; so no `#match' predicate is emitted.
;;
;; Removal plan: delete this whole block once we upgrade to an Emacs
;; that carries the bug#79687 fix (Emacs 31+, or a backport landing in
;; a future 30.x point release).
(eval-when-compile
