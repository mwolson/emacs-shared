...
Note: `C-c . .` currently doesn't work as of Emacs 30.1.
...
The recommended version is Emacs 30.1. The recommended ways to install Emacs for each OS are as follows.

### Install Emacs on Windows

- [Download and install emacs](https://ftp.gnu.org/gnu/emacs/windows/emacs-30/emacs-30.1-installer.exe).
- When done, you should verify that a file named `C:\Program Files\Emacs\emacs-30.1\bin\runemacs.exe` exists.
...
- Adjust some environment variables so that Emacs can be started successfully
  ...
  - In System Variables, double-click `Path` and make sure an entry for `C:\Program Files\Emacs\emacs-30.1\bin` is present at the end. If you see an entry for a different version of Emacs, change it to have this content instead and remove any duplicates of it.
  - Click OK
  - Relaunch any open Git Bash or MSYS2 windows

If upgrading:
- Unpin any pinned Emacs icons
- Change any existing desktop icons to point to `C:\Program Files\Emacs\emacs-30.1\bin\runemacs.exe`.
...
cd build/emacs
curl -o - https://ftp.gnu.org/gnu/emacs/emacs-30.1.tar.gz | tar -xzf -
cd emacs-30.1
./autogen.sh
./configure
make -j$(proc)
...
if uname | grep "MINGW64_NT" > /dev/null 2>&1; then
    # for MSYS2
    export PATH="~/emacs-shared/bin":"/c/Windows/System32/OpenSSH":"/c/Program Files/Emacs/emacs-30.1/bin":/ucrt64/bin:"/c/Program Files/Git/bin":"$PATH"
elif uname | grep "MSYS_NT" > /dev/null 2>&1; then
    # for Git Bash
    export PATH="/c/Windows/System32/OpenSSH":"$PATH"
fi
```
* Check `.bashrc` and update any `PATH` entries which have a different version of Emacs to instead point to `/c/Program Files/Emacs/emacs-30.1/bin` and restart Terminal.
...
Open `Start Menu -> Emacs`. This should point to the file `C:\Program Files\Emacs\emacs-30.1\bin\runemacs.exe`.
...
- Change "C:\Program Files\Emacs\emacs-30.1\bin\emacs.exe" to "C:\Program Files\Emacs\emacs-30.1\bin\runemacs.exe"
...
```sh
pushd extra/emacs
git fetch --depth 1 origin tag emacs-30.1
